class Score {
  ...
  void init(String fileName) {
    ...
    for (int i = 0; i < this.file.length; i++) {
      ...
      if (this.file[i].equals("#START")) {
        this.startPos = i;
      }
      if (this.file[i].equals("#END")) {
        this.endPos = i;
      }
    }
  ...
  void displayEditingPart() {
    ...
    stroke(255, 255, 0);
    if (this.editing) {
      stroke(255, 0, 255); //editing color.
    }
  ...
  void scrollY(int scroll) {
    ...
    if (this.notes.length > 5) {
      if (this.scrollY < -(this.notes.length - 5) * 5) {
        this.scrollY = -(this.notes.length - 5) * 5;
      }
    } else {
      if (this.scrollY < 0) {
        this.scrollY=0;
  ...
  void checkEditingPart(int x, int y) {
    ...
    if (!(x>unitX*4) || !(x<unitX*36) || !(y>unitY*7) || !(y<unitY*38)) {
      return;
    }
    if (this.editing) {
      return;
    ...
    for (int i = 0; i < this.notes.length+1; i++) {
      this.editing = false;
      if (y > unitY*(8+i*5+this.scrollY) && y < unitY*(12+i*5+this.scrollY) && x > unitX*4 && x < unitX*20) {
        this.editingPart = i * 2;
      }
      if (y > unitY*(8+i*5+this.scrollY) && y < unitY*(12+i*5+this.scrollY) && x > unitX*20 && x < unitX*36) {
        this.editingPart = i * 2 + 1;
      }
    }
  }
  ...
  void turnEditing() {
    if (!this.editing) {
      ...
      for (int i = 0; i < this.editingNotes.length; i++) {
      ...
      if (this.editingPart / 2 + 1 > this.notes.length) {
        ...
        for (int i = 0; i < this.EDIT_NOTE_NUM; i++) {
          newNotes[newNotes.length - 1][i + (this.editingPart % 2) * 8] = this.editingNotes[i];
        }
        for (int i = 0; i < this.EDIT_NOTE_NUM; i++) {
          newNotes[newNotes.length - 1][this.NOTE_NUM - 1 - (i + (this.editingPart % 2) * 8)] = 0;
        }
        ...
        for (int i = 0; i < this.EDIT_NOTE_NUM; i++) {
          this.notes[this.editingPart / 2][i + (this.editingPart % 2) * 8] = this.editingNotes[i];
        }
        for (int i = 0; i < this.notes.length; i++) {
          this.lines[i] = "";
          for (int j = 0; j < this.notes[i].length; j++) {
            this.lines[i] += str(this.notes[i][j]);
            if (j == this.notes[i].length - 1) {
              this.lines[i] += ",";
            }
          }
        }
  ...
  void edit(int x, int y) {
    ...
    for (int i = 0; i < this.NOTE_NUM/2; i++) {
      if (dist(unitX*(5+(this.editingPart%2)*16+i*2), unitY*(10+5*int(this.editingPart/2)+this.scrollY), x, y)<unitX*1) {
